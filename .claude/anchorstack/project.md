...
Note: the shared stack's `docker compose` invocation must run from its own checkout, `/Users/ben/repos/SpecHub` — **not** whichever git worktree the current session happens to be operating from (confirm the actual working dir baked into a running container via `docker inspect spechub-app-1 --format '{{index .Config.Labels "com.docker.compose.project.working_dir"}}'`; running `docker compose` from an unrelated worktree directory silently targets a different, empty compose project instead of erroring). To get a worktree branch onto the shared stack for live testing: since a branch can't be checked out in two worktrees/checkouts at once, `cd /Users/ben/repos/SpecHub && git checkout -b test/<branch> origin/<branch>` (a differently-named local ref pointing at the same commit — same workaround as this file's Multica planner/builder note above), then `docker compose up -d --build app` there. This switches the shared stack off `main` until switched back — confirm with the user first, since another concurrent session may expect it to reflect `main`.
