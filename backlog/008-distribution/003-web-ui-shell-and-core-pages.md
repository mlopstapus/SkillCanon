...
**Update (2026-08-05):** `005-governance-views-ui` shipped and archived (`backlog/005-governance/archive/005-governance-views-ui.md`) — the policy/objective UI gap above is closed; `/teams/[teamId]/policies` and `/objectives` are real, live-verified pages (scope tree, inherited/local split, create/edit/delete for both, all four policy enforcement types). This item's own Acceptance Criteria are not all closed yet, though: the full "create a team → create a project → create a policy → create a prompt → expand it → confirm the applied policy appears in the result" manual smoke test has not been run end-to-end in this session — only the team → policy portion was live-verified. Run the remaining project → prompt → expand portion before checking this item's Acceptance Criteria complete.
