...
- [ ] RLS policies enabled on `distribution.prompt_usage` and every other table this schema accumulates by the time this feature is built
- [ ] Every existing query against these tables (recordPromptUsage, getPromptUsageSummaryForProject, and whatever `008-distribution`'s own core features add) already filters by `organization_id` — audit against this feature, don't assume
- [ ] M3 negative test per resource type: a user in org A cannot read or write org B's usage rows by any query path

## Acceptance Criteria

- [ ] Cross-org access is denied for `distribution.prompt_usage`, proven by test
