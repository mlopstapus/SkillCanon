...
- [ ] `prompt_registry.prompts` table: `id`, `organization_id`, `name`, `description`, `is_deprecated`, `active_version_id` (nullable), `owner_type` (`"user" | "team"`), `owner_id`, `forked_from_skill_id` (nullable, self-referencing FK — lineage pointer, set only when created via fork), timestamps
- [ ] `(organization_id, name)` unique — **not globally unique**, correcting the current schema
- [ ] Invariant: `owner_id` must belong to the prompt's `organization_id` — a user (if `owner_type: "user"`) or a team (if `owner_type: "team"`)
- [ ] `createPrompt`'s owner is always the creating user (`owner_type: "user"`) — there is no direct "create as team-owned" path; a skill becomes team-owned only via `forkSkill` with `ownerType: "team"` (see `003-prompt-sharing.md`)
...
- [ ] Creating a prompt with an `owner_id` from a different organization (either owner type) is rejected
