...
status: open
dependencies: ["009-skill-chains.md", "backlog/004-app-shell-and-landing/002-app-shell-and-navigation.md"]
...
**Status: no Claude design mockup exists yet for these pages** (carried over unresolved from `007-workflow-orchestration/005-workflow-views-ui.md`). Pull the corresponding mockup(s) via the `claude_design` MCP server first and run the same gap-analysis pass against `009-skill-chains.md` before finalizing this file's Requirements — don't invent them from a description.
...
- [ ] A run-history view, sourced entirely from `listSkillChainRuns`/`getSkillChainRun` (`009-skill-chains.md`) — resolved step content (`system_message`/`user_message`) and each step's caller-self-reported status. There is no "run" action anywhere in this UI; the web UI never calls `startSkillChainRun`/`advanceSkillChainRun` and has no way to show a model's actual response, since Prompt Registry itself never receives one — real runs happen entirely client-side in whatever agent (Claude Code, another IDE) is executing the chain.
