...
- [x] [012 - Fix Legacy Backend Import Path Mismatch](archive/012-fix-legacy-backend-import-path-mismatch.md) — repo-hygiene bug discovered 2026-07-29 (`spechub_server` directory vs. `skillcanon_server` internal imports), same "not a convention decision, tracked here anyway" rationale as 011; already fixed by a separate commit (`e08f897`, 2026-08-02) before this item was formally closed out
- [x] [013 - Fix Non-Idempotent Workflow Schema Drop Migration](archive/013-fix-non-idempotent-workflow-schema-drop-migration.md) — migration-hygiene bug discovered 2026-08-02 while live-verifying `027-skill-chain-views-ui` (`0024_drop_workflow_schema.sql`'s `DROP TABLE` has no `IF EXISTS`, breaking `pnpm db:migrate` on any database that never had `workflow.workflows` in the first place), same "not a convention decision, tracked here anyway" rationale as 011/012
- [x] [014 - Fix Missing JWT_SECRET in Compose Env](archive/014-fix-missing-jwt-secret-compose-env.md) — compose-config bug discovered 2026-08-05 while live-verifying `031-governance-views-ui` (`docker-compose.yaml`'s `app` service has no `JWT_SECRET`, so a fresh self-hosted registration fails signing the session JWT even though the account was already created), same "not a convention decision, tracked here anyway" rationale as 011/012/013
