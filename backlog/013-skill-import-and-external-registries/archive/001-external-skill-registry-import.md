...
**Status note (036-external-skill-import, 2026-08-10):** The backend fetch/parse/create pipeline and the web UI's New Skill drawer "Import from link" mode are built and verified end-to-end against a real public repo (`anthropics/skills`, 16 real skills discovered and one imported live). This item's own title (`npx skills add`) describes the caller-facing shape of the feature, not a requirement that SkillCanon itself ship a dedicated CLI subcommand — no `cli/src/commands/add.ts`/REST route was built as part of this item, and per explicit product direction that's not required for it to be considered done: the web UI path *is* the caller-facing surface for this feature.
...
- [x] Importing a source with one real `SKILL.md`-shaped skill produces a new, correctly-owned skill in SkillCanon with matching markdown content and template files. Verified live via the web UI against `anthropics/skills`.
