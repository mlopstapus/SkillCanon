...
| `resolveAuditRows(db, orgId, requestingUserId, events)` | Resolves each event's resource/actor display name for rendering, deduped per page by distinct `(resourceType, resourceId)`/actor pair; falls back to the raw id when no resolver exists for a type or the resource/actor no longer exists (020-audit-log-ui) | Distribution (audit log UI) |
| `listAuditActorOptions(db, orgId, requestingUserId, retentionCutoff)` | Every distinct actor (resolved to a display name) that appears in the org's currently-retained events — not just currently-active members — powers the Audit Log page's Actor filter (020-audit-log-ui) | Distribution (audit log UI) |
