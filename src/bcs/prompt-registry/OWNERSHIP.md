...
| `prompt_registry.prompts` | `(organization_id, name)` unique. `owner_type`/`owner_id` — exactly one of a user or a team, never a project (PDR-016); mutable in place via `transferSkillOwnership` (PDR-019); PDR-016's who-can-own rule is unchanged, only whether the current owner can be reassigned. `source_url` (nullable, 013-skill-import-and-external-registries) — set only for a skill created via external import, never changed afterward |
