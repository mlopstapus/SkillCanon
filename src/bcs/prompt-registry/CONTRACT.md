...
| `forkSkill(db, actingUser: UserSummary, sourceSkillId, { ownerType, ownerId, name, description? }, auditContext?)` | Creates a new, independent skill **shell** under a new owner (a user or a team) with the caller-supplied `name`/`description`, stamped with `forkedFromSkillId` pointing back at the source for lineage/audit. No version content is copied — the fork starts with `activeVersionId: null`; its first version is authored separately through the normal `publishVersion` path, exactly like a blank-created skill. Rejects forking into an owner that already owns the source (FR-021), and rejects a duplicate `name` within the org (`DuplicatePromptNameError`). Superseded the original 020-prompt-sharing "copy the source's current active version verbatim, auto-generate a hash-suffixed name" behavior per [the 2026-08-15 editable-copy design](../../../docs/superpowers/specs/2026-08-15-editable-skill-copy-design.md) — a caller-editable copy needs an editable name up front and editable content afterward, not an instant unrenamable duplicate. | Distribution (route handlers) |
| `transferSkillOwnership(db, actingUser: UserSummary, skillId, { newOwnerType, newOwnerId }, auditContext?)` ([PDR-019](../../../docs/pdr/019-skill-ownership-transfer.md)) | Changes a skill's owner in place. An organization admin may transfer any in-organization skill; every non-admin must be authorized for both the current owner and the destination owner. Preserves the skill `id`, versions, subscriptions, project assignments, and fork lineage. | Distribution (route handlers, web UI) |
...
| `SkillOwnershipTransferred` | orgId, skillId, actorUserId, before/after ownerType and ownerId | Audit |
...
`expand()`'s output shape (`{ content, appliedPolicies, objectives }` since 032-skill-file-format-refactor) and the recursive-inclusion max depth (`MAX_INCLUDE_DEPTH`) are stable; increasing the depth limit is backward compatible, decreasing it is not. A skill's `ownerType`/`ownerId` can change in place via `transferSkillOwnership` ([PDR-019](../../../docs/pdr/019-skill-ownership-transfer.md)): an org admin may transfer any in-organization skill, while every non-admin must be authorized for both the current owner and the destination owner. This is the one exception to fork/subscribe being the only owner-facing operations. Every other identity (`id`, versions, subscriptions, project assignments, fork lineage) is unaffected by a transfer. A chain version's step resolution order is strictly sequential by step position, never dependency-graph order; every step resolves its own content independently with no per-step arguments (032-skill-file-format-refactor) — this is user-visible behavior IDEs build around and changing it requires a PDR, same as the equivalent guarantee `007-workflow-orchestration`'s `CONTRACT.md` previously stated.
