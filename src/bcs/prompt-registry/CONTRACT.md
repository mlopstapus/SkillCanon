...
| `transferSkillOwnership(db, actingUser: UserSummary, skillId, { newOwnerType, newOwnerId }, auditContext?)` ([PDR-019](../../../docs/pdr/019-skill-ownership-transfer.md)) | Changes a skill's owner in place. The skill's current owner or an organization admin may authorize the transfer. Preserves the skill `id`, versions, subscriptions, project assignments, and fork lineage. | Distribution (route handlers, web UI) |
