...
**Scope: page-level vs. in-page-section empties.** `AppState` applies when the entire page's primary content is the thing that's empty, loading, or errored — a whole list page with zero rows, a whole detail page that failed to load, a whole dashboard section with no data yet. It does **not** apply to a compact "no X yet" line inside one subsection of an otherwise-populated, multi-section detail page (e.g. a project detail page's own "No members yet." inside its Members card, sitting alongside populated Teams/Prompts/Usage sections on the same page). Retrofitting `AppState`'s centered, `py-16` layout into a small inline list slot would visually break the surrounding card, not improve consistency. These in-section notices still get a real accessible treatment (a `role="status"`/`aria-live="polite"` wrapper, not silence) — they just don't adopt `AppState`'s full layout. `src/app/(app)/projects/[id]/project-detail-view.tsx` and `src/app/(app)/prompts/[name]/prompt-detail-view.tsx` are the current examples of this category.
