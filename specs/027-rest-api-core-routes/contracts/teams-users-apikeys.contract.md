...
| GET | `/api/teams` | `listTeams(tx, orgId)` | Query `parentTeamId?` filters client-side; `flat?` reserved, currently always flat |
...
| GET | `/api/users` | `listUsers(tx, orgId)` | Query `teamId?` filters client-side |
