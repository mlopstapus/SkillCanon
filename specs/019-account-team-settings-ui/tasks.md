...
- [X] T037 [P] [US2] Write structural test `src/app/(app)/teams/team-form-drawer.test.tsx` — 4 tests covering all four modes, all passing
- [X] T038 [US2] Create `src/app/(app)/teams/team-form-drawer.tsx`: client component — create/edit/new-sub-team/insert-above modes per `contracts/account-team-settings-ui.md`'s Team form drawer table, surfacing errors inline. **Gap found**: `UpdateTeamFields`/`updateTeam` explicitly never touch hierarchy position (by design, separate from `reparentTeam`) — but FR-005 requires editing a team's parent too. Fixed by having the edit-mode submit call `reparentTeam` (via a new `newParentTeamId` param on `updateTeamAction`) only when the admin actively changes the parent selection. **Known limitation documented in `docs/stubs.md`**: no application function supports reparenting a team back to root (`parent_team_id → null`) — `reparentTeam` requires a non-null target — so the edit form's parent select omits a "— (root)" option for an already-root team instead of offering a control that would fail.
- [X] T039 [US2] Create `src/app/(app)/teams/actions.ts`: `"use server"` `createTeamAction`, `updateTeamAction` (+ `newParentTeamId` for the reparent-via-edit gap above), `insertTeamBetweenAction` — each resolves the acting user via `authenticateSession(authDb, cookieHeader)` before calling into `@/bcs/identity-access`
- [X] T040 [US2] Edit `src/app/(app)/teams/teams-explorer.tsx`: add admin-only Create (sidebar)/Edit/Insert-above/New-sub-team (header) buttons wired to `team-form-drawer.tsx` and `actions.ts`, gated on `user.role === "admin"`; on success, `router.refresh()`. **Gap found and fixed**: calling `useRouter()` directly inside the exported `TeamsExplorer` broke every structural test (`renderToStaticMarkup` has no App Router context to satisfy the hook — "invariant expected app router to be mounted"). Fixed by splitting into a pure `TeamsExplorerView` (takes `refresh: () => void` as a prop, what the tests render) and a thin `TeamsExplorer` wrapper that calls `useRouter()` and passes `router.refresh` down — the exact pattern this repo's own `app-navigation.tsx` already established (`NavigationList`/`AppNavigation`), just not one I'd initially followed.
- [X] T041 [US2] Manual verification: quickstart.md steps 2–5 — deferred to the same consolidated browser pass as T025

**Checkpoint**: User Stories 1 and 2 both work independently — full read + admin-write team structure management. (pending only the deferred manual browser check)
...
- [X] T069 [P] Run `pnpm typecheck` — clean throughout, including after the `isScopeAllowedForRole` client-bundle fix below
- [X] T070 [P] Run `pnpm lint` (`eslint . --fix` then plain `pnpm lint`) — clean
- [X] T071 Run the full suite (`pnpm vitest run`, not just the two feature dirs) — 119 files / 493 tests, all green
- [X] T072 Run `pnpm build` — **found and fixed a real production-build break**: `issue-key-drawer.tsx` ("use client") imported `isScopeAllowedForRole` as a real (non-type) value from the `identity-access` barrel, which pulled the whole server-side dependency graph (`createApiKey`, `inviteUser`, transitively `nodemailer`/`postgres`) into the browser bundle — `Module not found: Can't resolve 'tls'` (Node built-in, no browser build). Fixed by duplicating the one-line rule locally in that component instead of importing it; reverted the barrel export. Build passes cleanly afterward, both `/teams` and `/settings/api-keys` listed as dynamic routes.
- [X] T073 Manual verification + design parity, done together via a real browser session (`pnpm exec next dev -p 3001` against the persistent Postgres) rather than quickstart.md's numbered-step order — logged in as a real admin, then live-tested: create team, edit team (including the documented root-limitation placeholder), insert-above, new-sub-team, invite member (confirmed the logged "email delivery skipped" line with a real invite link), remove member (confirmed the exact `RemoveMemberConfirm` copy), unassigned-users reassignment (confirmed the key-auth-restored implication end-to-end at the backend level via T044's test, not re-verified live). **Found and fixed a real bug this way**: the sidebar tree sorted every team flatly alphabetically across the whole org, not in proper parent-then-children tree order — an unrelated sibling team could sort alphabetically *between* a parent and its own child, visually breaking the "tree." Fixed with a real depth-first `treeOrder()` function (siblings alphabetical, but a subtree always completes before the next sibling); added a regression test proving it (`teams-explorer.test.tsx`, the "Marketing"-sorts-between-"Engineering"-and-"Platform" fixture) since the original two-branch fixtures never had enough siblings for the bug to surface. Visual design (dark tokens, teal accent, drawer/modal chrome) matches `SkillCanon Settings.dc.html` closely. Did not click through `/settings/api-keys` live (covered instead by 6 passing structural tests + the clean production build) due to session rate-limit pressure encountered mid-verification.
- [X] T074 Reviewed `docs/stubs.md` — removed the "New sub-team CTA" row (confirmed wired and working live in the browser, not just by test); the root-reparent limitation row remains, genuinely still open
- [X] T075 Updated `backlog/002-identity-access/010-account-and-team-settings-ui.md` (below) — also filed `backlog/000-foundations/011-fix-missing-migration-snapshot-files.md` for the pre-existing migration-tooling gap found during T007 (Foundational), per this repo's convention of filing discovered gaps rather than only mentioning them in a commit message
