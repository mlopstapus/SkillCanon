...
- [ ] T020 [P] [US1] Write `renderToStaticMarkup` structural test `src/app/(app)/teams/page.test.tsx`: asserts the tree contains every team in a fixture hierarchy at the correct nesting, and that the default-selected team's Members tab lists its fixture members
- [ ] T021 [P] [US1] Write structural/behavioral test `src/app/(app)/teams/teams-explorer.test.tsx`: selecting a nested team shows its full root-to-team breadcrumb; typing into the filter narrows the visible rows; a non-admin sees no Create/Edit/Insert/New-sub-team controls and no invite/remove controls

### Implementation for User Story 1

- [ ] T022 [US1] Create `src/app/(app)/teams/page.tsx`: server component — resolve `authenticateSession(authDb, cookieHeader)` (redirect `/login` if none — mirrors `(auth)/welcome/page.tsx`'s pattern), fetch the org's full team list, the default-selected team's detail (own team), **and that team's member list** (existing `listByOrgAndTeam`-style read — required by T023's read-only Members tab) via the existing `listSubTeams`/`getTeamChain`/team-read functions, pass everything to `TeamsExplorer`
- [ ] T023 [US1] Create `src/app/(app)/teams/teams-explorer.tsx`: client component — indented tree from `parentTeamId` chains, client-side name filter (no server round-trip), Details/Sub-teams/Members tab switching (read-only in this phase — no mutation wired), breadcrumb display from each team's ancestor chain. The Sub-teams empty state's "New sub-team" CTA renders but stays admin-gated (hidden entirely for non-admins; for admins it renders without a working handler until T038/T040 wire it in US2) — do not leave it clickable-but-broken for anyone in this phase
- [ ] T024 [US1] Verify `src/app/(app)/_components/nav-model.ts`'s existing `/teams` entry needs no change (confirm only — it already points here)
- [ ] T025 [US1] Manual verification: quickstart.md step 1 (browse, filter, breadcrumb)

**Checkpoint**: User Story 1 is fully functional and independently testable — anyone can browse and inspect the hierarchy, with no mutation capability yet.
