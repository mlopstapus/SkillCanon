...
- **FR-006**: System MUST record each pruning run as its own new audit event — attributed to no human or API-key actor, tagged as system-originated, and carrying the count of events that run deleted (including zero) — so the pruning job's activity is itself visible in the trail rather than a silent background operation. The deletion and its recording event MUST succeed or fail together (never a deletion with no corresponding record, nor a record with no matching deletion), matching this codebase's existing guarantee that an audit write can never silently fail while the mutation it describes succeeds.
...
- The pruning job runs as a singleton per organization per scheduled interval — no two runs for the same organization execute concurrently. This feature does not itself define the concurrency-safety mechanism (e.g., an advisory lock); it follows this codebase's existing precedent for singleton-guarded jobs, a planning-level implementation concern rather than a business requirement.
