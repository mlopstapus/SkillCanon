...
- [ ] T008 [US1] Update `src/app/(app)/prompts/[name]/prompt-detail-view.test.tsx` to remove any assertions on the "Projects" button/`onOpenAssignProjects` prop and add an assertion that no requirement/enforcement control renders
...
- [ ] T018 [US3] Replace the `PromptDetailData` banner-related fields in `src/app/(app)/prompts/[name]/prompt-detail-view.tsx` with the `shareSummary: { teamCount, subscriberCount, copyCount }` shape (per data-model.md) and update the "Shared with…" pill to render "X teams · Y subscribers · Z copies" (depends on T017)
